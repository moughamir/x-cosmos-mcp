    ...
    return html`
      <div class="container mx-auto p-4">
        ...
        <div class="input-group">
          ...
          />
          <button @click="${this.pullModel}" class="pull-button">Pull Model</button>
        </div>

        <button @click="${this.fetchModels}" class="refresh-button">Refresh Models</button>

        <div class="model-list">
          ${this.models.length === 0
            ? html`<p>No Ollama models found. Ensure Ollama is running and models are pulled.</p>`
            : html`
                ${this.models.map(
                  (model) => html`
                    <div class="model-card">
                      <div>
                        ...
                        Size: ${(model.size / (1024 * 1024 * 1024)).toFixed(2)} GB<br>
