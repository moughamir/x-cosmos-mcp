...
class Worker:
    def __init__(self, worker_id: str, worker_pool: "WorkerPool", task_handlers: Dict[str, Callable[[Any], Awaitable[Any]]]):
    ...
    async def process_task(self, task: WorkerTask) -> WorkerResult:
        ...
        try:
            for attempt in range(max_retries + 1):
                try:
                    logger.debug(f"Worker {self.worker_id} processing task {task.task_id} (attempt {attempt + 1})")
                    ...
                    return WorkerResult(task.task_id, True, result, execution_time=execution_time)

                except Exception as e:
                    execution_time = time.time() - start_time
                    logger.warning(f"Worker {self.worker_id} attempt {attempt + 1} failed for task {task.task_id}: {e}")

                    if attempt < max_retries:
                        wait_time = min(2**attempt, 30)
                        logger.info(f"Worker {self.worker_id} retrying task {task.task_id} in {wait_time}s")
                        await asyncio.sleep(wait_time)
                        continue
                    else:
                        self.error_count += 1
                        logger.error(f"Worker {self.worker_id} failed task {task.task_id} after {max_retries + 1} attempts: {e}")
                        return WorkerResult(task.task_id, False, error=str(e), execution_time=execution_time)
...
class WorkerPool:
    def __init__(self, max_workers: int = 4, queue_size: int = 100, task_handlers: Dict[str, Callable[[Any], Awaitable[Any]]] = None):
    ...
    async def submit_task(self, task_type: str, data: Any, priority: int = 0) -> str:
        ...
        task = WorkerTask(task_id=task_id, task_type=task_type, data=data, priority=priority)
    ...
    async def _worker_loop(self, worker: Worker):
        while self.running:
            try:
                ...
                total_time = self.worker_stats["avg_execution_time"] * (self.worker_stats["completed_tasks"] + self.worker_stats["failed_tasks"] - 1)
                total_time += result.execution_time
                count = self.worker_stats["completed_tasks"] + self.worker_stats["failed_tasks"]
                self.worker_stats["avg_execution_time"] = total_time / count if count > 0 else 0
    ...
    async def _result_processor(self):
        while self.running:
            try:
                ...
                expired_tasks = [task_id for task_id, result in self.results.items() if current_time - result.execution_time > 3600]
    ...
    async def _health_check(self):
        ...
        for worker in self.workers:
            ...
            if worker.status == WorkerStatus.BUSY and worker.current_task and current_time - worker.current_task.created_at > 300:
...
async def initialize_worker_pool(max_workers: int, task_handlers: Dict[str, Callable[[Any], Awaitable[Any]]]):
    global worker_pool
    if worker_pool is None:
