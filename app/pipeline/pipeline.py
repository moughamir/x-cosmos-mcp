...
class MultiModelSEOManager:
    ...
    async def optimize_meta_tags(self, product_data: Dict[str, Any], quantize: bool = False) -> Dict[str, Any]:
        ...
        Current Description: {self._clean_html(product_data.get('body_html', ''), model, 500)}
        ...
        return await self._call_model_with_fallback(model, prompt, task_type=TaskType.META_OPTIMIZATION, quantize=quantize)
    
    async def rewrite_content(self, product_data: Dict[str, Any], quantize: bool = False) -> Dict[str, Any]:
        ...
        Original Description: {self._clean_html(product_data.get('body_html', ''), model, 2000)}
        ...
        return await self._call_model_with_fallback(model, prompt, task_type=TaskType.CONTENT_REWRITING, quantize=quantize)
    
    async def analyze_keywords(self, product_data: Dict[str, Any], quantize: bool = False) -> Dict[str, Any]:
        ...
        return await self._call_model_with_fallback(model, prompt, task_type=TaskType.KEYWORD_ANALYSIS, quantize=quantize)
        
    async def optimize_tags(self, product_data: Dict[str, Any], quantize: bool = False) -> Dict[str, Any]:
        ...
        return await self._call_model_with_fallback(model, prompt, task_type=TaskType.TAG_OPTIMIZATION, quantize=quantize)
    
    async def _call_model_with_fallback(self, model: str, prompt: str, task_type: TaskType, max_retries: int = 3, quantize: bool = False) -> Dict[str, Any]:
        """Call model with fallback to other models if needed"""
        for attempt in range(max_retries):
            current_model = model
            try:
                result = await self._call_ollama_model(current_model, prompt, quantize=quantize)
    ...
    async def _call_ollama_model(self, model: str, prompt: str, quantize: bool = False) -> Dict[str, Any]:
    ...
    async def batch_process_products(self, product_ids: List[int], task_type: TaskType, quantize: bool = False):
