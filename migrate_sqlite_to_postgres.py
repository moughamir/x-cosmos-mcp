...
async def migrate_products_table():
    ...
    sqlite_conn = await aiosqlite.connect(settings.paths.database.replace('postgresql://', ''))
    ...
    try:
        ...
        for product in products:
            try:
                ...
                for json_field in ['normalized_tags_json']:
                    if product_dict.get(json_field) and isinstance(product_dict[json_field], str):
                        try:
                ...
                await pg_conn.execute("""
                    INSERT INTO products (
                        ...
                        llm_model, llm_confidence, normalized_category, category_confidence,
        ...
        logger.info(f"✅ Successfully migrated {migrated_count}/{len(products)} products")
...
async def migrate_pipeline_runs():
    ...
    try:
        ...
        logger.info(f"✅ Successfully migrated {migrated_count}/{len(runs)} pipeline runs")
...
async def verify_migration():
    ...
    try:
        ...
        sample_product = await pg_conn.fetchrow("SELECT id, title FROM products LIMIT 1")
        if sample_product:
            logger.info(f"   Sample product: {sample_product['id']} - {sample_product['title'][:50]}...")
